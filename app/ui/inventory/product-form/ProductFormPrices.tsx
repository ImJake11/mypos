...
import ProductFormVAT from './components/ProductFormVat';
        ...
        <div className='w-full flex flex-col gap-4'>
            ...
            <div className={`w-full flex gap-2.5 flex-col md:flex-row`}>
                ...
                <div className='flex-1 flex flex-col'>
                    <ErrorMessage isShow={!costPrice} message='Cost price is required' />
                    <TextInputField value={costPrice.toString()} isNumeric={true} label='Cost Price' name='costPrice' onChange={handleText} placeholder='Cost Price' />
                </div>

                <div className='flex-1 flex flex-col'>
                    <ErrorMessage isShow={!sellingPrice} message='Tax is required' />
                    <TextInputField value={tax.toString()} isNumeric={true} label='Tax Rate' name='tax' maxLenght={3} onChange={handleText} placeholder='Tax Rate' />
                </div>

                <div className='flex-1 flex flex-col'>
                    <ErrorMessage isShow={!tax} message='Selling price is required' />
                    <TextInputField value={sellingPrice.toFixed(2)} isNumeric={true} label='Selling Price' name='sellingPrice' onChange={handleText} placeholder='Selling Price' />
                </div>
            </div>
            ...
            {/** tier button */}
            {bulkEnabled && <button className='bg-[var(--color-brand-primary)] rounded-[4px] h-[2.5rem] text-white p-[10px_15px w-[8rem]' onClick={() => dispatch(formAddBulkTier())}>Add tier</button>}
        </div>
    </ProductFormCard>
}
...
function CheckBox({ isChecked, onClick }: CheckboxProps) {
    return <div
        className={`grid place-content-center h-[1.2rem] w-[1.2rem] rounded-[3px] `}
        style={{
